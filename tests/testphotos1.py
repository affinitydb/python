...
def _selectDistinctGroups():
    ...
    _lGroupsRaw = _getpins("SELECT * FROM \"http://localhost/afy/class/testphotos1/user\";")
    _lGroupsIds = set()
...
def _randomTag(_pTagName, _pRatio=0.10):
    ...
    _lCount = lAffinity.qCount("SELECT * FROM \"http://localhost/afy/class/testphotos1/tag\" WHERE \"http://code.google.com/p/tagont/hasTagLabel\"='%s';" % _pTagName) # review: index by tag
    if 0 == _lCount:
        print ("adding tag %s" % _pTagName)
        lAffinity.q("INSERT (\"http://code.google.com/p/tagont/hasTagLabel\") VALUES ('%s');" % _pTagName)
    # Select an arbitrary number of 'photos', and tag them.
    for _iP in _getpins("SELECT * FROM \"http://localhost/afy/class/testphotos1/photo\";"):
        if random.random() <= _pRatio:
            lAffinity.q("INSERT (\"http://www.semanticdesktop.org/ontologies/2007/03/22/nfo#hasHash\", \"http://code.google.com/p/tagont/hasTagLabel\") VALUES ('%s', '%s');" % (_iP["http://www.semanticdesktop.org/ontologies/2007/03/22/nfo#hasHash"], _pTagName))
...
def _randomGroupPrivileges():
    ...
    _lTags = _getpins("SELECT * FROM \"http://localhost/afy/class/testphotos1/tag\";")
    ...
    for _iG in _lGroupIds:
        ...
        for _iR in _lRights:
            lAffinity.q("INSERT (\"http://code.google.com/p/tagont/hasTagLabel\", \"http://code.google.com/p/tagont/hasVisibility\") VALUES ('%s', '%s');" % (_iR["http://code.google.com/p/tagont/hasTagLabel"], _iG))
...
def _randomUserPrivileges():
    ...
    _lUsers = _getpins("SELECT * FROM \"http://localhost/afy/class/testphotos1/user\";")
    ...
    for _iU in _lUsers:
        ...
        for _iR in _lRights:
            lAffinity.q("INSERT (\"http://code.google.com/p/tagont/hasTagLabel\", \"http://code.google.com/p/tagont/hasVisibility\") VALUES ('%s', '%s');" % (_iR["http://code.google.com/p/tagont/hasTagLabel"], _iU["http://xmlns.com/foaf/0.1/mbox"]))
...
def _entryPoint():
    ...
    lAffinity.q("COMMIT;")
    lCntPhotos = lAffinity.qCount("SELECT * FROM \"http://localhost/afy/class/testphotos1/photo\";")
    ...
    for iU in lUsers:
        ...
        lCntUsersInGroup = lAffinity.qCount("SELECT * FROM \"http://localhost/afy/class/testphotos1/user\" WHERE \"http://xmlns.com/foaf/0.1/member/adomain:Group\"='%s';" % lGroup)
        print ("group %s contains %d users" % (lGroup, lCntUsersInGroup))
    lAffinity.q("COMMIT;")
    lCntUserGroups = len(_selectDistinctGroups())
    lCntUsers = lAffinity.qCount("SELECT * FROM \"http://localhost/afy/class/testphotos1/user\";")
    ...
    lCntPrivileges = lAffinity.qCount("SELECT * FROM \"http://localhost/afy/class/testphotos1/privilege\";")
    ...
    lTags = _getpins("SELECT * FROM \"http://localhost/afy/class/testphotos1/tag\";")
    ...
    lUsersOfInterest = _getpins("SELECT * FROM \"http://localhost/afy/class/testphotos1/privilege\"(%s) AS p JOIN \"http://localhost/afy/class/testphotos1/user\" AS u ON (p.\"http://code.google.com/p/tagont/hasVisibility\" = u.\"http://xmlns.com/foaf/0.1/mbox\");" % lFirstTagStr)
    # TODO: isolate and log as bug (same pattern seems to work elsewhere)
    #lUsersOfInterest = _getpins("SELECT * FROM \"http://localhost/afy/class/testphotos1/privilege\" AS p JOIN \"http://localhost/afy/class/testphotos1/user\" AS u ON (p.\"http://code.google.com/p/tagont/hasVisibility\" = u.\"http://xmlns.com/foaf/0.1/mbox\") WHERE (p.\"http://code.google.com/p/tagont/hasTagLabel\" IN (%s));" % lFirstTagsStr)
    ...
    def _countUserPhotos(_pUser):
        _lTagsP = _getpins("SELECT * FROM \"http://localhost/afy/class/testphotos1/privilege\" WHERE \"http://code.google.com/p/tagont/hasVisibility\"='%s';" % _pUser)
        ...
        if len(_lExpected_usPriv.difference(_lTags)) > 0:
            ...
            assert False
        _lTagsP = _getpins("SELECT * FROM \"http://localhost/afy/class/testphotos1/privilege\" AS p JOIN \"http://localhost/afy/class/testphotos1/user\"('%s') AS u ON (p.\"http://code.google.com/p/tagont/hasVisibility\" = u.\"http://xmlns.com/foaf/0.1/member/adomain:Group\");" % _pUser)
        #_lTagsP = _getpins("SELECT * FROM \"http://localhost/afy/class/testphotos1/privilege\" AS p JOIN \"http://localhost/afy/class/testphotos1/user\" AS u ON (p.\"http://code.google.com/p/tagont/hasVisibility\" = u.\"http://xmlns.com/foaf/0.1/member/adomain:Group\") WHERE u.\"http://xmlns.com/foaf/0.1/mbox\"='%s';" % _pUser)
        #_lTagsP = _getpins("SELECT * FROM \"http://localhost/afy/class/testphotos1/privilege\" WHERE \"http://code.google.com/p/tagont/hasVisibility\"='%s' UNION SELECT * FROM \"http://localhost/afy/class/testphotos1/privilege\" AS p JOIN \"http://localhost/afy/class/testphotos1/user\" AS u ON (p.\"http://code.google.com/p/tagont/hasVisibility\" = u.\"http://xmlns.com/foaf/0.1/mbox\") WHERE u.\"http://xmlns.com/foaf/0.1/mbox\"='%s';" % (_pUser, _pUser)) # this segfaults... should check why
        for _iP in _lTagsP:
            _lTags.add(_iP["http://code.google.com/p/tagont/hasTagLabel"])
