...
def _entryPoint():
    ...
    lPin1 = PIN({"http://localhost/mv/property/testcollections1/myprop1":"lPin1", "http://localhost/mv/property/testcollections1/mycoll1":lCollection1, \
        "http://localhost/mv/property/testcollection1/mycoll3":lCollection3 # Review: to be removed... \
        }).savePIN()
    ...
    checkPersistedProp("comparing reloaded testcollections1/mycoll1 to lCollection1 initializer", lPin1, "http://localhost/mv/property/testcollections1/mycoll1", lCollection1)

    # Remove a few elements.
    if True:
        del lCollection1[100:150]
        del lPin1["http://localhost/mv/property/testcollections1/mycoll1"][100:150]
        if 0 != cmp(lPin1["http://localhost/mv/property/testcollections1/mycoll1"], lCollection1):
            reportError("comparing in-memory testcollections1/mycoll1 to lCollection1, after removal of 50 elements")
        checkPersistedProp("comparing reloaded testcollections1/mycoll1 to lCollection1, after removal of 50 elements", lPin1, "http://localhost/mv/property/testcollections1/mycoll1", lCollection1)

    # Insert new elements "by position".
    # Review: currently mvstore doesn't return those eids... waiting for a fix...
    if False:
        ...
        for iE in lCollection2:
            ...
            lPin1["http://localhost/mv/property/testcollections1/mycoll1"].insert(2, iE) #50, iE)
        print ("after: %s" % lPin1)
        if 0 != cmp(lPin1["http://localhost/mv/property/testcollections1/mycoll1"], lCollection1):
            reportError("comparing in-memory testcollections1/mycoll1 to lCollection1, after insert by position")
        checkPersistedProp("comparing reloaded testcollections1/mycoll1 to lCollection1, after insert by position", lPin1, "http://localhost/mv/property/testcollections1/mycoll1", lCollection1)

    # Insert new elements "by eid".
    if True:
        lIndexPivot = 5
        lEidPivot = lPin1.getExtra("http://localhost/mv/property/testcollections1/mycoll1", pEpos=lIndexPivot).mEid
        for iE in lCollection2:
            PIN({PIN.SK_PID:lPin1.mPID, "http://localhost/mv/property/testcollections1/mycoll1":(iE, PIN.Extra(pOp=mvstore_pb2.Value.OP_ADD_BEFORE, pEid=lEidPivot))}).savePIN()
    ...
    if True:
        ...
        checkPersistedProp("comparing reloaded testcollections1/mycoll3 to lCollection3", lPin1, "http://localhost/mv/property/testcollection1/mycoll3", lCollection3b)
        while len(lCollection3b) > 1:
            del lPin1["http://localhost/mv/property/testcollection1/mycoll3"][0]
            del lCollection3b[0]
            if 0 != cmp(lPin1["http://localhost/mv/property/testcollection1/mycoll3"], lCollection3b):
                reportError("comparing in-memory testcollections1/mycoll3 to lCollection3b, after deletion of element")
            checkPersistedProp("comparing reloaded testcollections1/mycoll3 to lCollection3b, after deletion of element", lPin1, "http://localhost/mv/property/testcollection1/mycoll3", lCollection3b)
        del lPin1["http://localhost/mv/property/testcollection1/mycoll3"]
        try:
            if lPin1.has_key("http://localhost/mv/property/testcollection1/mycoll3") or lPin1.getExtra("http://localhost/mv/property/testcollection1/mycoll3"):
                reportError("retrieving testcollection1/mycoll3 after deletion of the property")
        except Exception as ex:
            #print repr(ex)
            pass
        lPinChk = PIN.createFromPID(lPin1.mPID)
        if lPinChk.has_key("http://localhost/mv/property/testcollection/mycoll3"):
            reportError("retrieving testcollection1/mycoll3 after deletion of the property, on reloaded pin")

    lMvStore.close()

class TestCollections1(MVStoreTest):
    "A basic test for collections."
    def execute(self):
        _entryPoint()
MVStoreTest.declare(TestCollections1)
