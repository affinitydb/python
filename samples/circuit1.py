...
ease of use thanks to Affinity references - both as a simple object mapping convenience,
...
would like to demonstrate that all this implies no compromise on performance."""
...
class Component(object):
    ...
    ctype = property(fget=lambda c: c.mPin["http://localhost/afy/property/1.0/circuit/component/type"], doc="Component type.")
    cx = property(fget=lambda c: c.mPin["http://localhost/afy/property/1.0/circuit/component/x"], doc="Component X position in the schema.")
    cy = property(fget=lambda c: c.mPin["http://localhost/afy/property/1.0/circuit/component/y"], doc="Component Y position in the schema.")
    inputs = property(fget=lambda c: c.mPin.get("http://localhost/afy/property/1.0/circuit/component/inputs"), doc="Component's input connections.")
    outputs = property(fget=lambda c: c.mPin.get("http://localhost/afy/property/1.0/circuit/component/outputs"), doc="Component's output connections.")
    @classmethod
    def create(cls, pType, pX, pY):
        return Component(PIN({"http://localhost/afy/property/1.0/circuit/component/type":pType, "http://localhost/afy/property/1.0/circuit/component/x":pX, "http://localhost/afy/property/1.0/circuit/component/y":pY}).savePIN())
...
class Circuit(object):
    ...
    def __init__(self, pName):
        ...
        lPins = PIN.loadPINs(self.mAffinity.qProto("SELECT * FROM \"http://localhost/afy/class/1.0/Circuit\"('%s');" % pName))
        self.mComponentsCache = {} # Review: for multi-user, may want to refine/remove this.
        if lPins != None and len(lPins) > 0:
            self.mPin = lPins[0]
            for iC in self.mPin["http://localhost/afy/property/1.0/circuit/components"]:
                lC = Component(PIN.createFromPID(iC))
                self.mComponentsCache[repr(lC.mPin.mPID)] = lC
        # Or create a new circuit.
        else:
            self.mPin = PIN({"http://localhost/afy/property/1.0/circuit/name":pName}).savePIN() # Note: It doesn't matter that we don't yet have components.
    def addComponents(self, pWidth, pHeight, pDensity):
        ...
        if False:
            for iY in xrange(pHeight):
                for iX in xrange(pWidth):
                    ...
                    self.mPin.addElement("http://localhost/afy/property/1.0/circuit/components", PIN.Ref.fromPID(lC.mPin.mPID))
                    self.mComponentsCache[repr(lC.mPin.mPID)] = lC
        else:
            lPins = []
            for iY in xrange(pHeight):
                for iX in xrange(pWidth):
                    lPins.append(PIN({
                        "http://localhost/afy/property/1.0/circuit/component/type":random.choice(Component.TYPES), 
                        "http://localhost/afy/property/1.0/circuit/component/x":Circuit.DISPLAY_MARGIN + iX * (Circuit.DISPLAY_WIDTH + Circuit.DISPLAY_MARGIN), 
                        "http://localhost/afy/property/1.0/circuit/component/y":Circuit.DISPLAY_MARGIN + iY * (Circuit.DISPLAY_HEIGHT + Circuit.DISPLAY_MARGIN)}))
            PIN.savePINs(lPins)
            if True:
                for iP in lPins:
                    self.mPin.addElement("http://localhost/afy/property/1.0/circuit/components", PIN.Ref.fromPID(iP.mPID))
                    self.mComponentsCache[repr(iP.mPID)] = Component(iP)
                self.mAffinity.mTxCtx.flush() # REVIEW: automate like in nodejs...
        ...
        for iC in xrange(lNumConnections):
            ...
            while lComp2.mPin.mPID == lComp1.mPin.mPID:
                ...
                if lNumConnC1 > 3:
                    ...
                    lCandidates = PIN.loadPINs(self.mAffinity.qProto( \
                        "SELECT * FROM \"http://localhost/afy/class/1.0/Circuit/Component#bypos\" AS cp JOIN \"http://localhost/afy/class/1.0/Circuit\"('%s') AS c ON (cp.afy:pinID = c.\"http://localhost/afy/property/1.0/circuit/components\")%s;" % \
                        (self.mPin["http://localhost/afy/property/1.0/circuit/name"], lCondition)))
                        # TODO: enable when bug 108 is fixed.
                        #"SELECT * FROM \"http://localhost/afy/class/1.0/Circuit/Component#bypos\"([%s, %s], [%s, %s]) AS cp JOIN \"http://localhost/afy/class/1.0/Circuit\"('%s') AS c ON (cp.afy:pinID = c.\"http://localhost/afy/property/1.0/circuit/components\")%s;" % \
                        #(lMinX, lMaxX, lMinY, lMaxY, self.mPin["http://localhost/afy/property/1.0/circuit/name"], lCondition))))
                    if 0 == len(lCandidates):
                        logging.debug("couldn't find a candidate for %s with exclusions %s" % (lComp1.mPin.mPID, lExcludedStr))
                        lComp2 = None
                        break
                    logging.debug("candidates for (%s,%s): %s" % (lComp1.cx, lComp1.cy, ' '.join(["(%s,%s)" % (iO["http://localhost/afy/property/1.0/circuit/component/x"], iO["http://localhost/afy/property/1.0/circuit/component/y"]) for iO in lCandidates])))
            ...
            if lComp2 and not lComp2.mPin.mPID == lComp1.mPin.mPID:
                lComp1.mPin.addElement("http://localhost/afy/property/1.0/circuit/component/outputs", PIN.Ref.fromPID(lComp2.mPin.mPID))
                lComp2.mPin.addElement("http://localhost/afy/property/1.0/circuit/component/inputs", PIN.Ref.fromPID(lComp1.mPin.mPID))
                logging.info("connected %s to %s" % (lComp1.pid, lComp2.pid))
        print ("Committing...")
        self.mAffinity.commitTx()
        print ("Done.")
    def connectComponent(self, pC, pTo):
        ...
        for iTo in pTo:
            if iTo.pid in lOutputs:
                continue
            pC.mPin.addElement("http://localhost/afy/property/1.0/circuit/component/outputs", PIN.Ref.fromPID(iTo.mPin.mPID))
            lC = self.mComponentsCache[iTo.pid]
            lC.mPin.addElement("http://localhost/afy/property/1.0/circuit/component/inputs", PIN.Ref.fromPID(pC.mPin.mPID))
    ...
    def disconnectComponent(self, pC):
        ...
        for iC in safeList(pC.inputs):
            ...
            if lOutputs and isinstance(lOutputs, PIN.Collection) and len(lOutputs) > 1:
                ...
                del self.mComponentsCache[repr(iC)].mPin["http://localhost/afy/property/1.0/circuit/component/outputs"]
        for iC in safeList(pC.outputs):
            ...
            if lInputs and isinstance(lInputs, PIN.Collection) and len(lInputs) > 1:
                ...
                del self.mComponentsCache[repr(iC)].mPin["http://localhost/afy/property/1.0/circuit/component/inputs"]
        if pC.mPin.has_key("http://localhost/afy/property/1.0/circuit/component/outputs"):
            del pC.mPin["http://localhost/afy/property/1.0/circuit/component/outputs"]
        if pC.mPin.has_key("http://localhost/afy/property/1.0/circuit/component/inputs"):
            del pC.mPin["http://localhost/afy/property/1.0/circuit/component/inputs"]
    def deleteComponent(self, pC):
        ...
        self.mPin["http://localhost/afy/property/1.0/circuit/components"].remove(pC.pid)
        pC.mPin.deletePIN()
    def setName(self, pName):
        "Change the name of the circuit."
        self.mPin["http://localhost/afy/property/1.0/circuit/name"] = pName
    name = property(fget=lambda c: c.mPin["http://localhost/afy/property/1.0/circuit/name"], fset=setName, doc="Circuit's name.")
    components = property(fget=lambda c: c.mComponentsCache, doc="Components of the circuit.")
    @classmethod
    def declareAfyClasses(cls):
        "Declare the Affinity classes used by the implementation."
        try:
            AFFINITY().qProto("CREATE CLASS \"http://localhost/afy/class/1.0/Circuit\" AS SELECT * WHERE \"http://localhost/afy/property/1.0/circuit/name\" IN :0 AND EXISTS(\"http://localhost/afy/property/1.0/circuit/components\");")
            AFFINITY().qProto("CREATE CLASS \"http://localhost/afy/class/1.0/Circuit/Component#bypos\" AS SELECT * WHERE \"http://localhost/afy/property/1.0/circuit/component/x\" IN :0(int) AND \"http://localhost/afy/property/1.0/circuit/component/y\" IN :1(int);")
        except:
            pass
    @classmethod
