...
if __name__ == '__main__':
    ...
    for iF in lOtherAnnFunctions:
        PIN({PIN.SK_PID:lPinAnn.mPID, "functions":(iF, PIN.Extra(pOp=mvstore_pb2.Value.OP_ADD_BEFORE, pEid=lEidPivot))}).savePIN()
    ...
    print ("\n8. Sort Ann's functions alphabetically, by just moving elements around in mvstore.")
    ...
    lPinAnn["age"] = 36.5
    PIN({"age":(1.5, PIN.Extra(pOp=mvstore_pb2.Value.OP_PLUS))}, __PID__=lNewPins[2].mPID).savePIN(); lNewPins[2].refreshPIN()
    lNewPins[0]["age"] = 37
    del lNewPins[1]["age"]
